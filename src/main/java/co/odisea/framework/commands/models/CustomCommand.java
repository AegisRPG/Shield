...
import co.odisea.features.commands.staff.MultiCommandCommand;
...
import co.odisea.framework.exceptions.preconfigured.*;
...
import co.odisea.models.nerd.NerdService;
import co.odisea.utils.*;
import co.odisea.utils.worldgroup.SubWorldGroup;
...
import static co.odisea.api.common.utils.TimeUtils.parseDate;
import static co.odisea.api.common.utils.TimeUtils.parseDateTime;
import static co.odisea.utils.Distance.distance;
import static co.odisea.utils.Nullables.isNullOrAir;
import static co.odisea.utils.StringUtils.an;
...
public abstract class CustomCommand extends ICustomCommand {
	...
	//TODO: Uncomment if doing minigame
	...
	public Player convertToPlayer(String value) {
		if (isCommandBlock() || isAdmin()) {
			...
			if ("@r".equals(value)) {
				...
				return RandomUtils.randomElement(PlayerUtils.OnlinePlayers.where().viewer(player).get());
			}
			if ("@s".equals(value))
				return player();
		}
...
//					.toList());
	...
	protected void help() {
		...
		methods.forEach(method -> {
			...
			String usage = "/" + getAliasUsed().toLowerCase() + (Nullables.isNullOrEmpty(path.value()) ? "" : " " + path.value());
