...
import co.odisea.api.common.utils.TimeUtils;
...
import static co.odisea.utils.PlayerUtils.isSelf;
...
public class VanishUser implements PlayerOwnedObject {
    ...
    public boolean canHideFrom(VanishUser viewer) {
        ...
        Shield.debug("[Vanish] %s can%s hide from %s (%s > %s)".formatted(
                getNickname(),
                canHideFrom ? "" : " not",
                viewer.getNickname(),
                priority,
    ...
    public void notifyDisabled(Setting setting, String action) {
        ...
        if (!new CooldownService().check(uuid, "vanish-notify-" + setting.name().toLowerCase(), TimeUtils.TickTime.SECOND.x(3)))
            return;
