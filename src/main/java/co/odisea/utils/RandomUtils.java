...
public class RandomUtils extends co.odisea.api.common.utils.RandomUtils {
	...
	public static int randomInt(int min, int max) throws InvalidInputException {
		try {
			return co.odisea.api.common.utils.RandomUtils.randomInt(min, max);
	...
	public static double randomDouble(double min, double max) throws InvalidInputException {
		try {
			return co.odisea.api.common.utils.RandomUtils.randomDouble(min, max);
		} catch (IllegalArgumentException exc) {
			throw new InvalidInputException(exc.getMessage());
		}
	}
