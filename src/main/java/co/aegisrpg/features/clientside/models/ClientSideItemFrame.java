package co.aegisrpg.features.clientside.models;

import co.aegisrpg.utils.ItemBuilder;
import co.aegisrpg.utils.NMSUtils;
...
public class ClientSideItemFrame implements IClientSideEntity<ClientSideItemFrame, ItemFrame, org.bukkit.entity.ItemFrame> {
    ...
    private ClientboundMapItemDataPacket getMapRenderPacket(Player player, CraftMapView view) {
        ...
        return new ClientboundMapItemDataPacket(view.getId(), view.getScale().getValue(), view.isLocked(), icons, new MapItemSavedData.MapPatch(0, 0, 128, 128, data.buffer));
    }

    @Override
    public @NotNull List<Packet<ClientGamePacketListener>> getUpdatePackets(Player player) {
        final List<SynchedEntityData.DataValue<?>> values = entity().getEntityData().packAll();
        if (values == null)
