...
import co.aegisrpg.features.mobheads.common.HeadConfig;
import co.aegisrpg.features.mobheads.common.MobHead;
import co.aegisrpg.features.mobheads.common.MobHeadVariant;
import co.aegisrpg.features.mobheads.variants.*;
import co.aegisrpg.models.mobheads.MobHeadChanceConfigService;
...
import static co.aegisrpg.api.common.utils.Nullables.isNotNullOrEmpty;
import static co.aegisrpg.api.common.utils.StringUtils.camelCase;
import static co.aegisrpg.utils.Nullables.isNotNullOrAir;
import static co.aegisrpg.utils.Nullables.isNullOrAir;
import static co.aegisrpg.utils.RandomUtils.randomElement;
...
public enum MobHeadType implements MobHead {
	...
	MobHeadType(
		...
		if (isNotNullOrEmpty(headId) || isNotNullOrAir(headType)) {
			this.baseSkull = isNullOrAir(headType) ? Shield.getHeadAPI().getItemHead(headId) : new ItemStack(headType);
			this.namedSkull = new ItemBuilder(this.baseSkull).name("&e" + getDisplayName() + " Head").lore("&3Mob Head").build();
		} else {
			if (!"PLAYER".equals(name()))
				Shield.warn("No generic skull for MobType " + camelCase(this) + " defined");
			this.baseSkull = null;
